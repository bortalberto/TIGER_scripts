...
class ch_reg_settings: # purpose: organize the Channel Configuration Register Settings in an array format which can be sent over Ethernet or optical link
   def __init__(self,
      ...
      "BranchEnableT": self.parameter_array[17],  ## BranchEnableT_param
      "BranchEnableEQ" : self.parameter_array[18]  ## BranchEnableEQ_param
      }
      sav_channel_chlist=[]
   ...
   def update_command_words(self):  # acr 2017-09-01
      if ( (self.command_code & 0xF) == 0x1 ): # acr 2017-09-01
         ...
         self.cmd_word1 = ((self.Ch_DebugMode & 0x3) << 24)  + ((self.TriggerMode & 0x3) << 16)
          ...
          self.cmd_word4 = (QdcMode << 24) + ((BranchEnableT) << 16) + ((BranchEnableEQ) << 8) + (TriggerMode2B )
...
class gemroc_cmd_LV_settings(object): # purpose: organize the GEMROC Configuration Register Settings in an array format which can be sent over Ethernet or optical link
   def __init__(self, TARGET_GEMROC_ID_param=0, command_string_param='NONE', number_of_repetitions_param=1, delay_save="time_delay_save"):
      ...
      for key in  config["LV"].keys():
          self.GEMROC_LV_conf_dict[key.upper()]=config["LV"].getint(key)

      for key in config.keys():
          if "LV." in key:
              if "{}".format(self.TARGET_GEMROC_ID) in key:
                  for key_int in config[key]:
                      self.GEMROC_LV_conf_dict[key_int.upper()] = config[key].getint(key_int)
...
class gemroc_cmd_DAQ_settings(object): # purpose: organize the GEMROC Configuration Register Settings in an array format which can be sent over Ethernet or optical link
   def __init__(self,
      ...
      self.DAQ_config_dict = {
        ...
        "EXT_nINT_B3clk":                           config["GLOBAL"].getboolean("external_clock"),
      ...
      self.cmd_word2 = ((self.DAQ_config_dict["L1_period_simulated"] & 0x3FF) << 20) + ((self.DAQ_config_dict["B3Clk_sim_en"] & 0x1) << 18) + ((self.DAQ_config_dict["Tpulse_generation_w_L1Chk_enable"] & 0x1) << 17) + ((self.DAQ_config_dict["Periodic_L1En"] & 0x1) << 16) + (
                  self.DAQ_config_dict["L1_scan_window_LOWER_edge"] & 0xFFFF)
      self.cmd_word1 = ((self.DAQ_config_dict["TP_period"] & 0x3FF) << 20) + ((self.DAQ_config_dict["Periodic_TP_EN_pattern"] & 0xF) << 16) + ((self.DAQ_config_dict["Enable_DAQPause_Until_First_Trigger"] & 0x1) << 15) + ((self.DAQ_config_dict["DAQPause_Set"] & 0x1) << 14) + (
                  (self.DAQ_config_dict["Tpulse_generation_w_ext_trigger_enable"] & 0x1) << 13) + ((self.DAQ_config_dict["EXT_nINT_B3clk"] & 0x1) << 12) + ((self.DAQ_config_dict["TL_nTM_ACQ"] & 0x1) << 11) + ((self.DAQ_config_dict["AUTO_L1_EN"] & 0x1) << 10) + (
                                   (self.DAQ_config_dict["AUTO_TP_EN"] & 0x1) << 9) + ((self.DAQ_config_dict["TP_Pos_nNeg"] & 0x1) << 8) + (self.DAQ_config_dict["EN_TM_TCAM_pattern"] & 0xFF)
      self.cmd_word0 = ((self.DAQ_config_dict["UDP_DATA_DESTINATION_IPPORT"] & 0xF) << 26) + ((self.DAQ_config_dict["number_of_repetitions"] & 0x3FF) << 16) + ((self.gemroc_cmd_code & 0xF) << 11) + ((self.DAQ_config_dict["target_TCAM_ID"] & 0x3) << 8) + (
                  (self.DAQ_config_dict["TO_ALL_TCAM_EN"] & 0x1) << 6)
      self.command_words = [ self.cmd_header,
                             self.cmd_word3,
                             self.cmd_word2,
                             self.cmd_word1,
