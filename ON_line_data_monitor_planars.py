...
class menu():
    def __init__(self):
        ...
        Label(self.Status_frame_2, text="Last L1 ", font=("Courier", 25)).pack()
        self.L1_frame = Frame(self.Status_frame_2)
        self.L1_frame.pack()
        self.last_L1_list = []
        for i in range(0, len(self.GEM_to_config)):
            if i < 11:
                ...
                riga = 1
            colonna = ((i) % 11) * 2 + 1
            self.last_L1_list.append(Label(self.L1_frame, text=" ---- ", width=6))
            self.last_L1_list[i].grid(row=riga, column=colonna)
            Label(self.L1_frame, text='ROC {}'.format(i), width=6, font=(12)).grid(row=riga, column=colonna - 1)


        ## L1 TS
        Label(self.Status_frame_2, text="Last L1 TS", font=("Courier", 25)).pack()
        self.L1_TS_frame = Frame(self.Status_frame_2)
        self.L1_TS_frame.pack()
        self.last_L1_ts_list = []
        for i in range(0, len(self.GEM_to_config)):
            if i < 11:
                riga = 0
            else:
                riga = 1
            colonna = ((i) % 11) * 2 + 1
            self.last_L1_ts_list.append(Label(self.L1_TS_frame, text=" ---- ", width=6))
            self.last_L1_ts_list[i].grid(row=riga, column=colonna)
            Label(self.L1_TS_frame, text='ROC {}'.format(i), width=6, font=(12)).grid(row=riga, column=colonna - 1)

        ## Wrong l1_ts-tcoarse
        Label(self.Status_frame_2, text="Wrong L1_ts - TCoarse", font=("Courier", 25)).pack()
        self.L1_TS_diff_frame = Frame(self.Status_frame_2)
        self.L1_TS_diff_frame.pack()
        self.last_L1_ts_diff_list = []
        for i in range(0, len(self.GEM_to_config)):
            if i < 11:
                riga = 0
            else:
                riga = 1
            colonna = ((i) % 11) * 2 + 1
            self.last_L1_ts_diff_list.append(Label(self.L1_TS_diff_frame, text=" ---- ", width=6))
            self.last_L1_ts_diff_list[i].grid(row=riga, column=colonna)
            Label(self.L1_TS_diff_frame, text='ROC {}'.format(i), width=6, font=(12)).grid(row=riga, column=colonna - 1)
...
class runner(Thread):
    ...
    def run(self):
        ...
        self.analyzer=analyzer(self.GEMROC_acquiring_dict.keys())
        for number, GEMROC in self.GEMROC_acquiring_dict.items():
            self.acquirer_list.append(GEMROC_runner(GEMROC, self.data_buffer[number], self.caller))
            self.decoder_list.append(GEMROC_decoder(GEMROC, self.data_buffer[number], self.caller, self.analyzer))
...
class analyzer():
    def __init__(self,GEMROCs_keys):
        self.GEMROCs_done={}
...
class GEMROC_decoder(Thread):
    def __init__(self, reader, data_buffer, caller2,analyzer):
        self.GEMROC_id = reader.GEMROC_ID
        Thread.__init__(self)
        self.running = True
        self.reader = reader
    ...
    def decode(self, pacchetto):
        ...
        for i in range(0, int(len(pacchetto) / 8)):
            ...
            if (((int_x & 0xE000000000000000) >> 61) == 0x6):
                ...
                # tipo = "h"
                self.caller2.last_L1_list[self.GEMROC_id]["text"] = LOCAL_L1_COUNT
                self.caller2.last_L1_ts_list[self.GEMROC_id]["text"] = LOCAL_L1_TIMESTAMP
            ...
            if (((int_x & 0xC000000000000000) >> 62) == 0x0):
                ...
                elif 1320 < LOCAL_L1_TS_minus_TIGER_COARSE_TS < 1370:
                ...
                elif LOCAL_L1_TS_minus_TIGER_COARSE_TS > 1370 and LOCAL_L1_TS_minus_TIGER_COARSE_TS < 1567:
                    self.noise_rate[tiger] += 1
                    self.noise_rate_rst[tiger] += 1

                if LOCAL_L1_TS_minus_TIGER_COARSE_TS < 1299 or LOCAL_L1_TS_minus_TIGER_COARSE_TS > 1567:
                    roll_ove_l1_ts = LOCAL_L1_TS_minus_TIGER_COARSE_TS+65536-LOCAL_L1_TS_minus_TIGER_COARSE_TS
                    if roll_ove_l1_ts < 1299 or roll_ove_l1_ts > 1567:
                        self.wrong_l1_ts = "Wrong"
                    else:
                        if self.wrong_l1_ts != "Wrong":
                            self.wrong_l1_ts = "Fine"
                else:
                    if self.wrong_l1_ts != "Wrong":
                        self.wrong_l1_ts = "Fine"
            ...
            if (((int_x & 0xF000000000000000) >> 60) == 0x4):
                ...
                if not self.first_packet:
                    ...
                    self.caller2.last_L1_ts_diff_list[self.GEMROC_id]["text"] = self.wrong_l1_ts
                ...
                if self.last_UDP_number != 0:
                    ...
                    for T in range(0, 8):
                        self.caller2.noise_list[self.GEMROC_id * 8 + T]["text"] = "{:.2e}".format(self.noise_rate[T] / (self.pkts_counter * 6.25e-9 * (1567-1420))) #Average over channels
    ...
    def log_perf(self, eff_list):
        ...
        for T in range(0, 8):
            ...
            json_body[0]["fields"]["Noise_rate"] = self.noise_rate_rst[T] / ((self.number_pkts_to_log+1) * 6.25e-9 * (1567-1370))
            json_body[0]["fields"]["Sig_rate"] = self.signal_rate_rst[T] / ((self.number_pkts_to_log+1) * 6.25e-9 * (1370-1320))
            self.caller2.send_to_db(json_body)
